...
const authMiddleware = (...args) => {
  ...
  return (0, import_debugLogger.withLogger)("authMiddleware", (logger) => async (req, evt) => {
    if (options.debug) {
      logger.enable();
    }
    logger.debug("URL debug", { url: req.nextUrl.href, method: req.method, headers: (0, import_utils.stringifyHeaders)(req.headers) });
    logger.debug("Options debug", { ...options, beforeAuth: !!beforeAuth, afterAuth: !!afterAuth });
    if (isIgnoredRoute(req)) {
      logger.debug({ isIgnoredRoute: true });
      console.warn((0, import_errors.receivedRequestForIgnoredRoute)(req.nextUrl.href, JSON.stringify(DEFAULT_CONFIG_MATCHER)));
...
const createDefaultAfterAuth = (isPublicRoute, isApiRoute) => {
  return (auth, req) => {
    if (!auth.userId && !isPublicRoute(req) && isApiRoute(req)) {
      ...
      return (0, import_redirect.redirectToSignIn)({ returnBackUrl: req.url });
    }
...
const appendDevBrowserOnCrossOrigin = (req, res) => {
  ...
  if (shouldAppendDevBrowser && !!location && (0, import_utils2.isDevelopmentFromApiKey)(import_clerkClient.SECRET_KEY) && (0, import_utils2.isCrossOrigin)(req.url, location)) {
